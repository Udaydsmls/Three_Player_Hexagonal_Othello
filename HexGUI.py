...
class OthelloGUI:
    def __init__(self, master):
        ...
        self.game.rl_agent_c = OthelloQLearningAgent(
            state_size=13 * 19, action_size=13 * 19, epsilon=0.115, decay_rate=0.999, gamma=0.9)
        self.game.rl_agent_c.load_q_table("othello_q_table.pickle")

        self.canvas = tk.Canvas(
            self.master, width=19 * CELL_SIZE, height=13 * CELL_SIZE)
        self.canvas.pack()
    ...
    def handle_random_player_turn(self):
        ...
        self.game.current_player_index = (
            self.game.current_player_index + 1) % 3
    ...
    def handle_hardcoded_player_turn(self):
        ...
        if moves:
            ...
            for move in moves:
                ...
                self.game.make_move(r, c, "B ")  # Simulate the move

                # Count how many opponent pieces are flipped
                flipped_pieces = sum(row.count(
                    "B ") for row in self.game.board) - sum(row.count("B ") for row in temp_board)
    ...
    def end_game(self):
        ...
        messagebox.showinfo(
            "Game Over", f"Player {winner} wins with {counts[winner]} disks!")
